...
class Admin {
	...
	public function admin_menu() {
		...
		if ( isset( $_GET['page'] ) && 'friends-migrations' === $_GET['page'] ) {
			add_submenu_page( 'friends', __( 'Migrations', 'friends' ), __( 'Migrations', 'friends' ), 'manage_options', 'friends-migrations', array( $this, 'render_admin_migrations' ) );
		}
	}
	 ...
	 */
	public function render_admin_migrations() {
		if ( ! current_user_can( 'manage_options' ) ) {
		...
		<div class="wrap">
				...
				<tbody>
					...
					<tr data-migration-id="<?php echo esc_attr( $id ); ?>">
						...
						<td class="migration-status">
							<?php Migration::render_status_badge( $status ); ?>
						</td>
						<td>
							<?php if ( ! empty( $status['in_progress'] ) ) : ?>
								<button type="button" class="button button-primary process-batch" data-migration-id="<?php echo esc_attr( $id ); ?>">
									<?php esc_html_e( 'Process Batch Now', 'friends' ); ?>
								</button>
							<?php else : ?>
								<button type="button" class="button button-secondary run-migration" data-migration-id="<?php echo esc_attr( $id ); ?>">
									<?php esc_html_e( 'Run', 'friends' ); ?>
								</button>
							<?php endif; ?>
							<?php if ( has_action( 'friends_migration_debug_' . $id ) ) : ?>
								<button type="button" class="button button-small toggle-debug" data-migration-id="<?php echo esc_attr( $id ); ?>">
									<?php esc_html_e( 'Debug', 'friends' ); ?>
								</button>
							<?php endif; ?>
						</td>
					</tr>
					<tr class="migration-debug-row" data-migration-id="<?php echo esc_attr( $id ); ?>" style="display: none;">
			 ...
			 */
			do_action( 'friends_migrations_debug', $statuses );
			?>
		</div>
